...
tools/docsnip/    content tooling (frontmatter validate, snippet check, llms.txt)
site/             throwaway Vite + React + MDX preview (docs + blogs); site/src/content-core is the shared parsing authority
site-artifacts/   GENERATED — per-project llms.txt (do not hand-edit)
...
4. **`site-artifacts/` is generated.** Run `uv run docsnip generate` after changing
   content and commit the result (per-project `*.llms.txt`). CI (`docsnip check`)
   fails if stale.
...
uv run docsnip generate                # regenerate site-artifacts/ (llms.txt)
