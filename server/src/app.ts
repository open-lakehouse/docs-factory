...
export function createApp(): Hono {
  ...
  // Function), so the browser needs CORS. ALLOWED_ORIGIN is set per deploy;
  // default to permissive in local dev only.
  const origin = process.env.ALLOWED_ORIGIN ?? "*";
  app.use("*", cors({ origin, allowHeaders: ["Content-Type", "Connect-Protocol-Version", "x-dev-persona"] }));

  // Liveness probe (Neon Functions can be evicted when idle; this warms them).
  app.get("/healthz", (c) => c.json({ ok: true }));

  const auth = selectProvider();
