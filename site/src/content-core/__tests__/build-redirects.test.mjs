...
test("redirectRoutes emits {src,dest,status:308} for each entry", () => {
  ...
  expect(out).toEqual([{ src: "/docs/delta/how-to/old", dest: "/docs/delta/how-to/read", status: 308 }]);
});

test("redirectRoutes throws when dest is not a known route (typo guard)", () => {
  expect(() => redirectRoutes({ "/old": "/docs/delta/how-to/typo" }, ROUTES)).toThrow(/not a known route/);
});
